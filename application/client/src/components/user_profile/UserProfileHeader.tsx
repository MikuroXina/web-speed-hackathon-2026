...
export const UserProfileHeader = ({ user }: Props) => {
  ...
  // 画像の平均色を取得します
  const handleLoadImage = useCallback(async (ev: SyntheticEvent<HTMLImageElement>) => {
    const { FastAverageColor } = await import("fast-average-color");
    const fac = new FastAverageColor();
    const { rgb } = await fac.getColorAsync(ev.target as HTMLImageElement, { mode: "speed" });
    setAverageColor(rgb);
    fac.destroy();
  }, []);
  ...
  return (
    <header className="relative">
      <div
        className={`h-32 ${averageColor ? `bg-(--calculated-color)` : "bg-cax-surface-subtle"}`}
        style={
          averageColor
            ? {
                "--calculated-color": averageColor,
              }
            : {}
        }
      ></div>
